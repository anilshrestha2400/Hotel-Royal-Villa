...
if (isset($_GET['fetch_rooms'])) {
    ...
    //check in check out filter validation
    if ($chk_avail['checkin'] != '' && $chk_avail['checkout'] != '') {
        ...
        if ($checkin_date == $checkout_date) {
            echo "<h3 class='text-center text-danger'>You cannot Check-in and Check-out in same day!!!</h3>";
            exit;
        } else if ($checkout_date < $checkin_date) {
            echo "<h3 class='text-center text-danger'>ICheck-out date is less than Check-in date!!!</h3>";
            exit;
        } else if ($checkin_date < $today_date) {
            echo "<h3 class='text-center text-danger'>Check-in date is less than today's date!!</h3>";
            exit;
        }
    }
    ...
    //count no of rooms and output variable to store rooms card
    $count_rooms = 0;
    $output = "";
...
if (isset($_POST['search_room'])) {
    ...
    $room_res = select("SELECT * FROM `rooms` WHERE `name`=? AND `removed`=? ORDER BY `id` DESC", [$frm_data['name'],0], 'si');
    ...
    if (binarySearch($room_names, $value) == true) {
        echo <<<room
            <div class='card mb-4 border-0 shadow' >
                <div class='row g-0 p-3 align-items-center'>
                    <div class='col-md-5 mb-lg-0 mb-md-0 mb-3'>
                    ...
                    <div class='col-md-5 px-lg-3 px-md-3 px-0'>
                        ...
                        <div class='quantity'>
                            <h6 class='mb-1'>Quantity of Room:
                            <span class='badge bg-light text-dark text-wrap'>
                                $room_data[quantity] 
                            </span>
                            </h6>
