    ...
    <div class="container">
        <div class="row">
            ...
            $query = "SELECT bo.*, bd.* FROM `booking_order` bo INNER JOIN `booking_details` bd ON bo.booking_id=bd.booking_id 
             ...
             OR (bo.booking_status='cancelled')) AND
             (bo.user_id=?)
            ...
            while ($data = mysqli_fetch_assoc($result)) {
                ...
                if ($data['booking_status'] == 'booked') {
                    ...
                    $status_bg = "bg-danger";
                    if ($data['refund'] == 0) {
                        $btn = "<button type='button' class='btn btn-primary rounded btn-sm shadow-none'>Refund in Process!!!</button>";
                    } else {
                        $btn = "<a href='generate_pdf.php?gen_pdf&id=$data[booking_id]' class='btn btn-dark btn-sm fw-bold shadow-none'>Download PDF</a>";
                    }
                }
            ...
            <div class="modal-content">
                <form id="review-form">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="text-end">
                            <button type="submit" class="btn btn-dark  btn-smshadow-none">SUBMIT</button>
                        </div>
                    </div>
    ...
    <script>
        function cancel_booking(id) {
            if (confirm('Are you sure to cancel booking?')) {
                ...
                xhr.open("POST", "ajax/cancel_booking_crud.php");
